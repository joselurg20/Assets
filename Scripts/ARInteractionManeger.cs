...
public class ARInteractionManeger : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0)
        {
            ...
            {
                Touch touchtTwo = Input.GetTouch(1);
                if(touchOne.phase == UnityEngine.TouchPhase.Began || touchtTwo.phase == UnityEngine.TouchPhase.Began) 
                {
                    initialTouchPos = touchtTwo.position - touchOne.position;
                }
                if(touchOne.phase == UnityEngine.TouchPhase.Moved || touchtTwo.phase == UnityEngine.TouchPhase.Moved)
                {
                    Vector2 currentTouchPos = touchtTwo.position - touchOne.position;
                    float angle = Vector2.SignedAngle(initialTouchPos, currentTouchPos);
                    item3DModel.transform.rotation = UnityEngine.Quaternion.Euler(0, item3DModel.transform.eulerAngles.y - angle, 0);
